...
def plot_real_time(fig,axarr,processed_data,freqs,vmin,vmax,median_data,maxSamples=10000,skiptime=25,skipch=1,sampleSize=1./125.,cmap='Reds',show_norm=False, show_power_filter=False, threshold=2e+17, i=0):
    ax=axarr
    ...
    plt.pause(.3)
    fig.savefig('../thesis_plotting_results/with_normalization_'+str(i)+".png")
...
def main(argv):
    ...
    while(True):
        ...
        if((len(new_median_data_filenames) > 0) | (len(new_scaled_data_filenames) > 0)):
            plot_real_time(fig,axarr,all_processed_data, metadata['freqs'],args.vmin,args.vmax,median_data=all_median_data,sampleSize=metadata[u'SAMPLING_TIME'],cmap=args.cmap,show_norm=args.show_normalization, show_power_filter=args.show_power_filter, threshold=args.threshold, i=i)
